...
const supabase = createClient(
  "https://dxthcumunhjmpalfbgxr.supabase.co",
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImR4dGhjdW11bmhqbXBhbGZiZ3hyIiwicm9sZSI6ImFub24iLCJpYXQiOjE2OTg2NDc4MDAsImV4cCI6MjAxNDIyMzgwMH0.sbpeGxXAsL8afNWlKqVjMhwZzG5B_qOyOc-pBZH377w",
  {}
);
...
function getRandomInt(min, max) {
  ...
  const randomNumber =
    Math.floor((randomBytes.readUInt32LE(0) / 0xffffffff) * range) + min;
  ...
  if (validateAmount(amount, 10) && validate(gameName)) {
    ...
    if (userdetails) {
      if (userdetails["coins"] >= amount) {
        var update_status = await updateBalance(
          userdetails["uuid"],
          0 - amount
        );
    ...
    if (userdetails) {
      ...
      if (gamedetails != null) {
        ...
        if (userdetails["coins"] >= gamedetails["stake"]) {
          var updatestatus = await updateBalance(
            userdetails["uuid"],
            0 - gamedetails["stake"]
          );
...
async function createping(gameid, hash, res) {
  var gamedetails = await db.collection("games").findOne({
    id: gameid,
    $or: [{ player1_hash: hash }, { player2_hash: hash }],
  });
...
app.post("/validatedeposit", (req, res) => {
  ...
  if (validate([uuid, token, deposit_token])) {
    ...
    res.send({
      error: true,
      message:
        "Unknown Error Please Contact Helpline if you money debited from your bank account.",
    });
  }
});
            ...
            if (withdrawlstatus) {
              db.collection("transactions")
                .insertOne({
                  ...
                  amount: amount,
                  message:
                    "withdrawal request has been created successfully. The requested funds will be credited to your designated payment method within the next 48 hours.",
  ...
  if (gamedetails["move"] == "p1") {
    ...
    if (safebox.includes(newposition)) {
      ...
      if (updatestats.modifiedCount > 0) {
        (gamedetails[coin] = newposition), (gamedetails["dice"] = "p2");
        gamedetails["move"] = "-";
    ...
    if (safebox.includes(newposition)) {
      ...
      if (updatestats.modifiedCount > 0) {
        (gamedetails[coin] = newposition), (gamedetails["dice"] = "p1");
        gamedetails["move"] = "-";
        gamedetails["lastaction"] = time;
  ...
  if (gamedetails != null) {
    if (gamedetails["player1_hash"] == hash) {
      ...
      if (gamedetails["dice"] == "p1") {
        ...
        var posiblemove = findpossiblemove(
          gamedetails["p1_p1_l"],
          gamedetails["p1_p2_l"],
          gamedetails["p1_p3_l"],
          gamedetails["p1_p4_l"],
...
app.post("/login_otp", function (req, res) {
  ...
  if (validate(mobilenum)) {
    if (mobilenum.length == 10) {
      var generated_otp =
        mobilenum != "9985515054" ? generateRandom4DigitNumber() : 3579;
        ...
        .then(function (data) {
          ...
          if (data.data.length > 0) {
            ...
            if (data.data[0]["validity"] < 4) {
              ...
              if (
                data.data[0]["mobile_num"] == mobilenum &&
                otp == data.data[0]["otpvalue"] &&
                data.data[0]["otpvalue"]
              ) {
